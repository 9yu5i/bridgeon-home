...
(() => {
  ...
  // (see readCategoryFromPage below) — one ~200 kB document per card. Two
  // things keep that cost off the shopper's back: this cache, and the partial
  // read in fetchProductPage.
  //
  ...
  const hydrateCards = async () => {
    ...
    await mapLimit(cards, FETCH_CONCURRENCY, async (card) => {
      ...
      if (cached?.category && !needsBrand) {
        card.dataset.wishCategory = cached.category;
        applyFilters();
        return;
      }
