...
$(function(){
	...
	$(".only_number_for_chk_ea").bind("blur",function(){
		...
		if( $(this).val() == "0" || $(this).val() == "" ){
			openDialogAlert("Please enter 0 or more.",400,140,function(){$el.val(max);$el.focus();});
			$(this).val(max);
		}
		$(this).trigger("change");
	});
	...
	$(".only_number_for_chk_ea").bind("keyup change",function(){
		...
		if((val < min || val > max) && val != 0){
			openDialogAlert("Quantity must be between " + min + " and " + max +".",400,140,function(){$el.val(max);$el.focus();});
			$(this).val(max);
		}
		// select 박스에 값 전달
