/**
Editorial Section (maon page) JS
 **/
...
(() => {
  ...
  const renderProductCard = (pick = {}) => {
    ...
    const brand = escapeHtml(pick.brand || "");
    const price = escapeHtml(pick.price || "");
    ...
    const thumb = pick.image
      ? `<img src="${escapeHtml(pick.image)}" alt="${productName}" loading="lazy" />`
      : "img";

    const goodsSeq = Number.parseInt(pick.goodsSeq, 10);
    const addToCartAttr = Number.isFinite(goodsSeq)
      ? ` onclick="parent.displayAddToCartQuickview(this, ${goodsSeq}, event); this.blur(); return false;"`
      : "";
    ...
    return `
      <article class="editor-card-product" data-product-detail-link="${detailUrl}">
        ...
        <button type="button" class="editor-product-add" aria-label="Add ${productName} to cart"${addToCartAttr}>+</button>
      </article>
    `;
  };
  ...
  const renderSharedEditors = () => {
    ...
    track.innerHTML = sharedEditors
      .map((editor, editorIndex) => {
        ...
        const suppliedTheme = toClassToken(editor.theme || "");
        const theme = editorCardThemes.includes(suppliedTheme)
          ? suppliedTheme
          : editorCardThemes[editorIndex % editorCardThemes.length];
        ...
        const job = escapeHtml(editor.job || "");
        const ctaUrl = escapeHtml(editor.ctaUrl || "editors-pick/editors-pick.html");
        const isActive = editorIndex === 0;

        const avatarMarkup = editor.photo
          ? `<img src="${escapeHtml(editor.photo)}" alt="${name}" loading="lazy" />`
          : "<i></i>";
        const avatarClass = editor.photo ? "editor-avatar has-photo" : `editor-avatar editor-avatar--${avatar}`;
        const flagMarkup = editor.flagImage
          ? `<img src="${escapeHtml(editor.flagImage)}" alt="" />`
          : "";
        const flagClass = editor.flagImage ? "flag has-photo" : `flag flag--${flag}`;
            ...
            <div class="editor-card-intro">
              <aside class="editor-card-profile">
                <div class="editor-profile-avatar">
                  <span class="${avatarClass}" aria-hidden="true">${avatarMarkup}</span>
                  <b class="${flagClass}" aria-hidden="true">${flagMarkup}</b>
                  <h3 class="editor-profile-name">${name}</h3>
                </div>
                <a class="editor-card-cta" href="${ctaUrl}">SHOP THE PICKS &rarr;</a>
              </aside>
