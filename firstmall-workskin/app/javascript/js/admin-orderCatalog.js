	...
	// 2024-10-25 hwlee : bak 처리
	function get_catalog_ajax_bak(){
		if	(loading_status == 'n'){
			loading_status	= 'y';
	...
	function custom_btn_direct_open(){
		$("span.btn-direct-open").hide();
		$("span.btn-direct-open").parent().parent().hide();
		$("span.btn-direct-open").each(function(idx, item){
			setTimeout(function(){
				orderViewOnOff('open', item);
			},100)
		});
	...
	// 2024-10-25 hwlee : bak
	function orderViewOnOff_bak(openType, thisObj){
	...
	function orderViewOnOff(openType, thisObj){
		...
		var order_seq	= $(thisObj).attr('data-order_seq');
		$.get('view?no='+order_seq+"&pagemode="+pagemode+"&shipping_provider_seq="+shipping_provider_seq, function(data) {
			nextTr.find('div.order_info').html(data);
			//$("#order-summary").find(".oth").hide();
		});
		nextTr.removeClass('hide');
		$(thisObj).addClass("opened");
	}
