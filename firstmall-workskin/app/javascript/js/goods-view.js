...
// 퀵뷰 등 AJAX로 삽입된 <script src> 를 jQuery가 내부적으로 dataType:'script'로
// 다시 불러올 때 기본적으로 cache:false 를 적용해서 매번 ?_=timestamp 를 붙여
// 매번 네트워크에서 새로 받아온다. goods.option.0.2.js/goods-options.js 등은
// 배포 시에만 바뀌는 정적 파일이므로 캐시를 허용해 불필요한 재다운로드를 막는다.
...
$(document).ready(function(){
	...
	$("#direct_store").bind("click",function(){
		var grp_seq = $(".goods_spec_shipping select[name='shipping_method'] option:selected").attr('grp_seq');
		$.ajax({
	...
	$("#shipping_detail_info").bind("click",function(){
		var selected_shipping_method = $(".goods_spec_shipping select[name='shipping_method'] option:selected");
		var grp_seq		= selected_shipping_method.attr('grp_seq');
		var nation		= selected_shipping_method.attr('nation');
		var set_seq		= selected_shipping_method.val();
		var prepay_info	= $("#shipping_prepay_info").val();
		const shipping_set_code = selected_shipping_method.attr('shipping_set_code');
		let direct_store = 'N';
	...
	*/
	...
	$(".addPresent").bind("click",function(){
		...
		var selected_shipping_method = $(".goods_spec_shipping select[name='shipping_method'] option:selected");
		var shipping_set_code = selected_shipping_method.attr('shipping_set_code');
		var nation = selected_shipping_method.attr('nation');
	...
	window.addEventListener('scroll', function() {
		const form = document.querySelector('form[name="goodsForm"]');
		if (!form) return;
