$(function(){
    ...
    // 옵션 컬러 스와치 업로드
    $(document).on("change", '.colorSwatchInput', function(){
        var fileInput   = this;
        var tr          = $(this).closest('tr');
        var optionSeq   = tr.find("input[name='option_seq[]']").val();

        if(!fileInput.files || !fileInput.files[0] || !optionSeq) return;

        var formData = new FormData();
        formData.append('colorSwatch', fileInput.files[0]);
        formData.append('tmpSeq', scObj.tmp_seq);
        formData.append('optionSeq', optionSeq);

        $.ajax({
            url         : '../goods_process/save_option_color_swatch',
            type        : 'post',
            data        : formData,
            contentType : false,
            processData : false,
            ...
            success     : function(res){
                if(res && res.status == 1){
                    var td = tr.find('.colorSwatchTd');
                    td.find("input[name='custom_color_swatch']").val(res.filePath);
                    td.find('.colorSwatchPreview').attr('src', res.filePath).show();
                    td.find('.colorSwatchDel').show();
                }else{
                    alert(res && res.msg ? res.msg : '업로드에 실패하였습니다.');
                }
                fileInput.value = '';
            },
            error       : function(){
    ...
    // 옵션 컬러 스와치 삭제
    $(document).on("click", '.colorSwatchDel', function(){
console.log("CHECK");
        var btn         = this;
        var tr          = $(this).closest('tr');
        var optionSeq   = tr.find("input[name='option_seq[]']").val();
        if(!optionSeq) return;
        if(!confirm('컬러 스와치 이미지를 삭제하시겠습니까?')) return;

        $.ajax({
            url         : '../goods_process/delete_option_color_swatch',
            type        : 'post',
            data        : { tmpSeq: scObj.tmp_seq, optionSeq: optionSeq },
            dataType    : 'json',
            success     : function(res){
                if(res && res.status == 1){
                    var td = tr.find('.colorSwatchTd');
                    td.find("input[name='custom_color_swatch']").val('');
                    td.find('.colorSwatchPreview').attr('src', '').hide();
                    $(btn).hide();
                }else{
                    alert(res && res.msg ? res.msg : '삭제에 실패하였습니다.');
                }
            },
            error       : function(){
                alert('삭제에 실패하였습니다.');
