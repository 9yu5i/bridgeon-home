	...
	required_field['KOREA_INPUT']		= [ 'recipient_input_user_first_name',
	...
	required_field['INTERNATIONAL_STATE']		= [ 'international_address',
	...
	required_field['ORDER']				= [ 'order_user_first_name',
	...
	ticket_required_field['INTERNATIONAL_INPUT'] = [ 'international_recipient_phone_input',
											];
	ticket_required_field['KOREA']				= [ 'recipient_user_name',
											'recipient_email',
											'recipient_cellphone[]'
										];
	ticket_required_field['INTERNATIONAL']		= [ 'international_recipient_phone'
										];
	ticket_required_field['ORDER']				= [ 'order_user_first_name',
											'order_user_last_name',
											'order_email',
										];

	let _isExpressShipping = false; 
	...
	function requiredChange(type,mode){
		...
		const STATE_REQUIRED_COUNTRIES = ["USA", "U.S.A", "CANADA", "IRELAND", "AUSTRALIA", "HONG KONG"];
		if (STATE_REQUIRED_COUNTRIES.includes(add_nation) && _isExpressShipping) {
			add_nation = "INTERNATIONAL_STATE";
		} else if (add_nation !== "KOREA") {
			add_nation = "INTERNATIONAL";
		}
	...
	function succ_input(type){
		...
		if(type != 'order'){ // 배송지 정보 수동적용
			...
			var add_nation					= $("#address_nation").val();
			var recipient_input_user_name	= $("input[name='recipient_input_user_first_name']").val() + " " + $("input[name='recipient_input_user_last_name']").val();
			...
			if(add_nation == 'KOREA'){
				...
				var phone_obj		= $("input[name='recipient_input_phone']").val();
				...
				var address2 = $("input[name='international_address_input_2']").val();
				if (address2) address += "|" + address2; // hwlee 2025-9-3 주석처리
				var address3 = $("input[name='international_address_input_3']").val();
				if (address3) address += "|" + address3; // hwlee 2025-9-3 주석처리
				...
				var phone_input = $("input[name='recipient_input_phone']").val();
				...
				if(type != 'ticket'){
					...
					$(".recipient_address").html([address.split("|")[0], address.split("|")[1], address.split("|")[2], town_city, county, country].filter(Boolean).join(', '));
				}

				$("input[name='international_recipient_phone']").val(phone_input);
				$(".delivery_info .phone").html(phone_input);
			}
			...
			$(".order_user_name").html($("input[name='order_user_first_name']").val() + " " + $("input[name='order_user_last_name']").val());
			$(".order_phone").html($("input[name='order_phone']").val());
	...
	function delivery_apply_input(){
		...
		var nameParts = recipient_user_name.trim().split(" ");
    		var first_name = nameParts[0];
    		var last_name = nameParts.length > 1 ? nameParts[nameParts.length - 1] : '';
    		$("input[name='recipient_input_user_first_name']").val(first_name);
    		$("input[name='recipient_input_user_last_name']").val(last_name);
		...
		if(add_nation == 'KOREA'){
			...
			var phone_obj					= $("input[name='recipient_phone']").val();
			...
			$("input[name='recipient_input_phone']").val(phone_obj);
		}else{
			...
			var gl_phone					= $("input[name='international_recipient_phone']").val();

			$("input[name='international_address_input']").val(gl_address.split("|")[0]);
			$("input[name='international_address_input_2']").val(gl_address.split("|")[1]);
			$("input[name='international_address_input_3']").val(gl_address.split("|")[2]);
			...
			$("input[name='recipient_input_phone']").val(gl_phone);
		}
	...
	function check_shipping_nation(){
		...
		$("#nation_image").attr("src","/admin/skin/default/images/common/icon/nation/" + nation + ".png");
	...
	// State 입력란 노출 체크 2025-07-25
	function check_fields_for_country(){
		var nation = $("#address_nation").val();
		if (!nation) nation = 'U.S.A';
		
		setPhoneCodeByCountry(nation);

		if (_isExpressShipping) {
			$("input[name='international_county_text_input']").hide();

			if (nation in STATE_REQUIRED_COUNTRIES) {
				$("input[name='international_county_input']").parent().parent().show();
				$("input[name='international_county_input']").addClass('pilsu');
				$("input[name='international_county_input']").attr('required', true);
				updateStatesList(STATE_REQUIRED_COUNTRIES[nation]);
			} else {
				$("input[name='international_county_input']").parent().parent().hide();
				$("input[name='international_county_input']").removeClass('pilsu');
				$("input[name='international_county_input']").removeAttr('required');
			}

		} else {
				$("input[name='international_county_text_input']").parent().show();
				$("input[name='international_county_text_input']").show();
				$("input[name='international_county_input']").parent().parent().hide();
				$("input[name='international_county_input']").removeClass('pilsu');
				$("input[name='international_county_input']").removeAttr('required');
		...
		const TAX_ID_COUNTRIES = ["GERMANY", "FRANCE", "ITALY", "SPAIN", "POLAND", "CZECH", "SLOVAKIA", "AUSTRIA"];
		if (TAX_ID_COUNTRIES.includes(nation)) {
			$("input[name='international_tax_id_input']").parent().show();
		} else {
			$("input[name='international_tax_id_input']").parent().hide();
		}

		check_signature_checkbox_visibility();
		updateTaxBillingMethod();
	}

	function updateTaxBillingMethod() {
		var country = $("#address_nation").val().replace(/\./g, "").toUpperCase();

		const $taxMethod = $("input[name='tax_billing_method']");
		const $stateTax = $("input[name='state_tax_input']");

		const UPS_DDP_TAX_BILLING_COUNTRIES = ["CANADA", "UK"];

		if (country === "USA") {
			$taxMethod.val('ddp');
			$stateTax.val(15); //hardcoded tax % for USA
			return;
		}

		if (_isExpressShipping && UPS_DDP_TAX_BILLING_COUNTRIES.includes(country)) {
			$taxMethod.val('ddp');
		} else {
			$taxMethod.val('ddu');
		}
	...
	function check_signature_checkbox_visibility() {
		var nation = $("#address_nation").val();

		const SIGNATURE_AVAILABLE_COUNTRIES = ["U.S.A", "USA", "CANADA", "GREECE", "NETHERLANDS", "NORWAY", "DENMARK",
		"MONACO", "BELGIUM", "SWEDEN", "SWITZERLAND", "IRELAND", "UNITED KINGDOM", "ITALY", "FRANCE", "FINLAND",
		"JAPAN", "MACAO", "SINGAPORE", "TAIWAN", "HONG KONG"];

		if (SIGNATURE_AVAILABLE_COUNTRIES.includes(nation)) {
			if (_isExpressShipping) { 
				$("input[name='signature_checkbox']").parent().parent().show();
			} else {
				$("input[name='signature_checkbox']").parent().parent().hide();
			}
		} else {
			$("input[name='signature_checkbox']").parent().parent().hide();
		}
	...
	function updateFreeShippingIcon() {
		const priceText = $(".total_delivery_shipping_price").text();
		const priceValue = parseFloat(priceText.replace(/[^\d.]/g, "")) || 0;

		$(".free-shipping-icon").toggleClass("hide", priceValue !== 0);
	}

	$(document).on("change", ".shipping_radio input[name='ship_set_list']", function () {
		const isExpress = $(this).attr("ship_set_code") === "quick";
		_isExpressShipping = isExpress;
	});

	// 주(State) 드롭다운 + 검색
	document.addEventListener("DOMContentLoaded", function () {
		$("#actionFrame").on("load", function () {
			updateFreeShippingIcon();
		});

		addStateDropdownListeners();
		addCountryDropdownListeners();
		setAddressLineRequirements();
		populateCountryCodeSelector();
	});

	function setAddressLineRequirements() {
		const addressInput = $("input[name='international_address_input']");
		const wrongInputMsg = $("#wrongAddressMsg");

		// 영문자, 숫자 및 일부 특수문자만 허용됩니다
		const allowedRegex = /^[A-Za-z0-9 .,()*&#+\-_~\/:;‘“]*$/;

		if (_isExpressShipping) {
			addressInput.attr("maxlength", "35");
		} else {
			addressInput.removeAttr("maxlength");
		}

		addressInput[0].addEventListener("input", function (e) {
			if (_isExpressShipping) {
				const isValidLength = this.value.length <= 35;
				const isValidChars = allowedRegex.test(this.value);
	...
	function selectState(el) {
		document.getElementById('stateSearchInput').value = el.textContent;
		stateOptionsList.classList.add('options-hidden');
		document.getElementById("searchInputHidden").value = el.dataset.code;

		if (el.dataset.tax) {
			document.getElementById('stateTax').value = el.dataset.tax;
			order_price_calculate();
		}
	}
	...
	function order_price_calculate(){
		...
		if (currentState === lastState) {
			//SKIP: no need to re-calculate order price
			return;
		}
	...
	function check_typereceipt()
	{
		...
		if(obj.val() > 0){
			// 주문자 휴대폰
			phone = $("input[name='order_user_phone']").val() != '' ? $("input[name='order_user_phone']").val() : $("input[name='recipient_input_phone']").val();
		...
		// 현금영수증 신청일 경우
			...
			$("input[name='creceipt_number[0]']").val(phone);
	...
	*/
	...
	function area_close_chk(){
		...
		if($(".order_info_detail").css('display') == 'block'){
			...
			var phone = $("input[name='order_user_phone']").val();
	...
	function copy_order_info(){
		if ($("input[name='order_user_name']").val()) {
    			const nameParts = $("input[name='order_user_name']").val().trim().split(" ");
    			const first_name = nameParts[0];
    			const last_name = nameParts.length > 1 ? nameParts[nameParts.length - 1] : '';
    
			$("input[name='recipient_input_user_first_name']").val(first_name);
			$("input[name='recipient_input_user_last_name']").val(last_name);
		} else if ($("input[name='order_user_first_name']").val() || $("input[name='order_user_last_name']").val()) {
    			$("input[name='recipient_input_user_first_name']").val($("input[name='order_user_first_name']").val());
    			$("input[name='recipient_input_user_last_name']").val($("input[name='order_user_last_name']").val());
		}
		
		if ($("input[name='order_user_phone']").val() != '') {
			$("input[name='recipient_input_phone']").val( $("input[name='order_user_phone']").val() );
		}
		$("input[name='recipient_input_email']").val( $("input[name='order_email']").val() );
		order_price_calculate();
	}
	...
	function copy_delivery_info(){
		if($("input[name='order_user_name']").attr('readonly') != 'readonly'){
			$("input[name='order_user_name']").val( $("input[name='recipient_input_user_first_name']").val() + " " + $("input[name='recipient_input_user_last_name']").val());
		}

		$("input[name='order_user_phone']").val( $("input[name='recipient_phone']").val() );
	...
	$(window).load(function(){
		...
		$("input#copy_order_info").bind("click",function(){
			if( $(this).attr("checked") ){
				...
				$("input[name='recipient_input_user_first_name']").val("");
				$("input[name='recipient_input_user_last_name']").val("");
				$("input[name='recipient_input_phone']").val("");
		...
		$("#pay").on("click",function(e){
			...
			if (!$("input[name='ics_checkbox']").is(":checked")) {
				alert("You must agree to the ICS (Invoice Convenience Service).");
				e.preventDefault();
				return false;
			}
			...
			if (_isExpressShipping) {
				...
				const isValidLength = value.length <= 35;
				const allowedRegex = /^[A-Za-z0-9 .,()*&#+\-_~\/:;‘“]*$/;
				const isValidChars = allowedRegex.test(value);

				if (!isValidLength || !isValidChars) {
					e.preventDefault();
					addressInput.focus();
					alert("Address format is invalid. Please use only English letters, numbers, spaces, and allowed symbols (max 35 characters).");
					return false;
				}
			}
			...
			// 주문자 전화번호가 비어 있는 경우, 검증을 위해 수취인 전화번호로 설정
			if ($("input[name='order_user_phone']").val() == "" && $("input[name='recipient_input_phone']").val() != "") {
				$("input[name='order_user_phone']").val($("input[name='recipient_input_phone']").val());
			}
			...
			if(gl_mobile=='1'){
				...
				if ( $.inArray(sel_payment, target_actionFrame) > -1 ) {
					// 카카오페이, 페이코, 네이버페이
					f.attr("target","actionFrame");
				...
				} else if ( $.inArray(sel_payment, target_actionFrame) > -1 ) {
					// 카카오페이, 페이코, 네이버페이
					f.attr("target","actionFrame");
					$("iframe[name='actionFrame']").hide();
				} else {
		...
		if ( is_members == true && is_order_present === false) {
				...
				$.ajax({
					...
					'success' : function(res){
						if(res){
							...
							if (res.recipient_user_first_name && res.recipient_user_last_name) {
								$("input[name='recipient_input_user_first_name']").val(res.recipient_user_first_name);
								$("input[name='recipient_input_user_last_name']").val(res.recipient_user_last_name);
							} else if (res.recipient_user_name) {
								const nameParts = res.recipient_user_name.trim().split(" ");
								const first_name = nameParts[0];
								const last_name = nameParts.length > 1 ? nameParts[nameParts.length - 1] : '';
							...
							$("input[name='recipient_input_phone']").val( res.recipient_phone );
			...
			if (typeof default_add_nation != 'undefined' && default_add_nation.length > 0 && default_add_nation !== 'KOREA') {
				...
				var international_address_line2 = $("input[name='international_address_input_2']").val();
				var international_address_line3 = $("input[name='international_address_input_3']").val();
				...
				var international_address = [
				    international_address,
				    international_address_line2,
				    international_address_line3,
				    international_town_city,
				    international_county,
				    international_postcode,
				    international_country
