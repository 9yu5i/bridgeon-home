...
$(function() {  
	...
	$('#boad_write_btn').live('click', function() {
		boardaddFormDialog(boardwriteurl, 1500, 1000, getCurrentBoardName() + ' 게시글 등록','false');
	});
	...
	$("a[viewlinkurl]").live("click",function(event){
		...
		boardaddFormDialog(viewlink, 1500, 1000, getCurrentBoardName() + ' 게시글 보기','false');
	});

	// 게시글 수정
	$(":input[name=boad_modify_btn]").live("click", function() {
		var seq = $(this).attr("board_seq");
		boardaddFormDialog(boardmodifyurl+seq, 1500, 1000, getCurrentBoardName() + ' 게시글 수정','false');
	});

	// 게시글 답변
	$(":input[name=boad_reply_btn]").live("click", function() {
		var seq = $(this).attr("board_seq"); 
		boardaddFormDialog(boardreplyurl+seq, 1500, 1000, getCurrentBoardName() + ' 게시글 답변','false');
	});
		...
		if(confirm("정말로 댓글을 삭제하시겠습니까? ")) {
			$.ajax({
				...
				'success' : function(res){
					if(res) {
						if(res.result == true){
							openDialogAlert(res.msg,'400','140',function(){boardaddFormDialog(returnurl+'#cwriteform', 1500, 1000, getCurrentBoardName() + ' 게시글 보기','false');});
						}else{
		...
		if(confirm("정말로 선택된 댓글을 삭제하시겠습니까? ")) {
			$.ajax({
				...
				'success' : function(res){
					if(res) {
						if(res.result == true){
							openDialogAlert(res.msg,'400','180',function(){boardaddFormDialog(returnurl+'#cwriteform', 1500, 1000, res.board_title + ' 게시글 보기','false');});
						}else{
